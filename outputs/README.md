...
A multiplayer fantasy finance game for groups of friends. Each player picks three stocks and allocates a virtual £300 pot across them at the start of a season. The leaderboard tracks everyone's portfolio value in real time — including live currency conversion for global stocks. At the end of the season, final standings are archived to an all-time Medal Table.
...
2. Players visit the link and sign in with Google to submit their name, three stock picks, and how they want to allocate their £300 virtual pot (min £50, max £150 per pick; must total exactly £300)
3. When everyone is ready, the admin locks current prices as the baseline and starts the game
4. Prices update automatically every 5 minutes during market hours; FX cross-rate pairs update at the same time so multi-currency portfolios stay accurate; a daily snapshot is saved by a cron job each weekday evening
...
- Each player allocates a virtual £300 pot: minimum £50 and maximum £150 per pick, adjustable in £10 increments, must total exactly £300
- Live ticker search powered by Yahoo Finance autocomplete — any equity or ETF globally, with exchange and currency shown in the dropdown
...
- Admin can edit or remove any player's picks and reallocations before the game starts

**Live game**
- Leaderboard sorted by current portfolio value (£), refreshed every 5 minutes
- Per-player portfolio cards showing current value and gain/loss per stock position
- Multi-currency support: any Yahoo Finance global stock tracked via live GBP cross-rates (e.g. GBPUSD=X) — currency movements affect standings throughout the season
- UK stocks priced in GBX (pence) are automatically converted to £
- All Stocks tab ranking every pick by % gain with visual performance bars
- Dashboard tab with an actual £ portfolio value chart and a stacked bar breakdown by stock
...
Any equity or ETF available on Yahoo Finance — globally. The currency is detected automatically and all portfolio values are converted to GBP in real time via Yahoo Finance cross-rate pairs.

- **US stocks and ETFs** — priced in USD; converted at live GBPUSD rate
- **London Stock Exchange** — use the `.L` suffix (e.g. `BP.L`, `HSBA.L`, `AZN.L`); prices in GBX (pence), automatically divided by 100
- **European stocks** — Euronext, Xetra, etc.; priced in EUR, converted via GBPEUR
- **Japanese stocks** — Tokyo Stock Exchange; priced in JPY, converted via GBPJPY
- **Hong Kong stocks** — HKEX; priced in HKD, converted via GBPHKD
